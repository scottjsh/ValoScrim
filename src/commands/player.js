...
async function player(client, msg, args) {
  ...
  if (player && player.link_id && !configs.includes("--fast")) {
    ...
    if (!obj.errors) account = obj.data;
  }
  ...
  if (player && !configs.includes("--fast")) {
    ...
    for (let prop in matches) {
      ...
      if (result) results.push(result.attacker == match.attacker);
    }
  ...
  if (player && !player.link_elo) player.link_elo = 0;

  if (player && !player.win_rate) player.win_rate = 0;
  ...
  const embed2 = new EmbedBuilder()
    ...
    .setDescription(
      `${
        user ? userMention(user_id) : msg.author
      } 님의 내전 정보 ${EmbedWhiteSpace()}`
    )
    .addFields([
      {
        name: "승률",
        value: player.matches
          ? `${player.matches_won}/${player.matches} \`(${(
              player.win_rate * 100
            ).toFixed(0)}%)\``
          : "0/0 (0%)",
        inline: true,
  ...
  if (account) embed2.setImage(account.card.wide);
