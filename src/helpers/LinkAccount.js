...
function LinkAccount({ name, tag, puuid, msg, user = false }) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // if (!guild && msg) guild = msg.guild;

      if (!user && msg) user = msg.author.id;
      ...
      // guild.members.fetch(user).then(async (member) => {
      //   if (mmr.elo) {
      ...
      //     console.log(`역할 업데이트됨: ${account.name}`);
      //   }

      //   member
      //     .setNickname(account.name)
      //     .then(() => {
      //       console.log(`닉네임 설정됨: ${account.name}`);
      //     })
      //     .catch((err) => {
      //       console.log("닉네임 설정 오류", err);
      //     });
      // });
